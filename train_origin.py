...
def main():
    ...
    # cfg는 모든 module의 yaml, --cfg 로 제공한 yaml (vae or mld)과 base yaml을 포함해서 모두 가지고 있다!
    ...
    # set seed
    pl.seed_everything(cfg.SEED_VALUE)  # default 1234
    ...
    # cfg에 datasets의 정보도 포함시킨다.
    # 그리고, 진짜 dataset을 return한다.
    # datasets = HumanML3DDataModule(...)
    ...
    # create model
    model = get_model(cfg, datasets[0])     # class MLD 생성
    ...
    if cfg.TRAIN.PRETRAINED_VAE:    # VAE 있을 때 - VAE load
    ...
    if cfg.TRAIN.RESUME:
        ...
        # datasets[0]은 HumanML3DDatasetModule의 instance인데, 해당 instance의 self.Dataset을 불러오는데, self.Dataset은 Text2MotionDatasetV2이다. (VAE 학습에서)
